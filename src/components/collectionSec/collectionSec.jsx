        ...
        <section className={clsx("lg:pt-20 lg:pb-[88px] py-16", bgColor)}>
            <div className="container">
                <div className='text-center'>
                    <TitleComponent size='small' className='collection-title-prefix mb-4 text-black'>Popular Collection</TitleComponent>
                        ...
                        {CollectionData.map((item, index) => {
                            return (
                                <Link to={"/"} key={index} className="flex mx-auto w-max justify-center items-center md:gap-x-10 sm:gap-x-5 gap-x-3 group" onMouseEnter={() => handleMouseEnter(item.color)} onMouseLeave={() => handleMouseEnter('bg-white')} >
                                    <TitleComponent type="h1" className={clsx("collection-title" + (index + 1), "md:translate-x-16 text-black lg:text-7xl md:text-[56px] text-[32px] leading-[120%] font-medium duration-500 md:group-hover:translate-x-14")}>{item.heading}</TitleComponent>
